...
<template>
  <div class="fixed top-0 inset-x-0 bg-white z-[100] px-6 h-[65px] hidden md:block"
    :class="isExpandSidebar ? 'pl-[265px]' : 'pl-[79px]'" style="box-shadow: 0 0 40px 0 rgb(82 63 105 / 10%)">
    <div class="flex items-stretch justify-end h-full">
      <div class="flex items-stretch">
        <div class="flex items-center relative">
            ...
            <div
              ...
              <span class="">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                  class="w-6 h-6 text-primary" viewBox="0 0 24 24" version="1.1">
          ...
          <div v-if="isShowNotification" class="absolute top-0 left-0 will-change-transform w-[350px] bg-white rounded"
            style="
                  transform: translate3d(-272px, 65px, 0px);
                  box-shadow: 0 0 50px 0 rgba(82, 63, 105, 0.15);
                ">
            <form>
              <div class="flex flex-col pt-8 pb-6 bg-cover bg-no-repeat rounded-t bg-primary"
                style="background-image: url(/assets/media/misc/bg-1.jpg)">
                <h4 class="flex justify-center items-center rounded-t">
                  <span class="text-white">Notifikasi </span>
                  <span v-if="unreadNotifications" class="rounded px-1 py-0.5 bg-success text-white font-bold ml-2">
                    {{ unreadNotifications }} baru
                  </span>
                </h4>
              </div>
              <div v-if="notifications.length" class="h-[300px] relative list-none my-4">
                <router-link v-for="notification in notifications" :key="notification.id" :to="notification.link">
        ...
        <div class="flex items-center">
          <div
            ...
            <span class="shrink-0 relative rounded w-9 aspect-square flex items-center bg-[#e5e9ff] justify-center">
