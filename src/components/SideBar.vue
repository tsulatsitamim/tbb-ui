  ...
  <div :class="{
    ...
    <div class="px-[25px] items-center justify-between h-[67px] hidden md:flex">
      ...
      <button class="h-[58px] flex items-center" @click="toggleExpandSidebar">
        <span class="hover:text-primary transition-all hover:rotate-180"
    ...
    <div id="sidebar-container" class="my-4 overflow-scroll custom-scroll h-full">
      <ul class="list-none">
        ...
        <template v-for="(item, index) in routes" :key="index">
          <router-link v-if="item.path && !item.children" :key="item.path"
            v-slot="{ href, route, navigate, isActive, isExactActive }" :to="item.path">
            <li
              ...
              <a :href="href" class="flex items-center hover:text-current relative" @click="navigate"><span
                  :class="!isExpandSidebar && 'md:w-min'" class="flex w-[35px]" v-html="item.icon"></span><span
                  :class="!isExpandSidebar && 'md:hidden'">{{ item.name }}</span>
                <span v-if="item.badge" :id="item.badge"
          ...
          <li v-if="item.children" :key="item.path" class="flex flex-col"
            ...
            <a href="javascript:;"
              ...
              <span class="flex w-[35px]" :class="!isExpandSidebar && 'md:w-min'" v-html="item.icon"></span>
            ...
            <div class="overflow-hidden transition-all duration-500 ease-in-out bg-white text-[#3f4254]"
              :class="activeSubmenu.includes(index.toString()) ? 'max-h-[800px]' : 'max-h-0'">
              <span class=""></span>
              <ul class="">
                <router-link v-for="child in item.children" :key="child.path"
                  v-slot="{ href, route, navigate, isActive, isExactActive }" :to="child.path">
                  <li class="pl-10 pr-[25px] min-h-[40px] flex items-center hover:bg-slate-100 hover:text-primary"
                    :class="[isExactActive && 'text-primary bg-slate-100']">
                    <a :href="href" class="flex items-center space-x-3 hover:text-current" @click="navigate"><span
                        class="w-1 h-1 rounded-full bg-[#3f4254]"></span><span :class="!isExpandSidebar && 'md:hidden'">{{
                          child.name }}</span></a>
                  </li>
                </router-link>
              </ul>
          ...
          <li v-if="!item.icon" class="mt-5 px-[25px] flex items-center h-10">
            <h4 class="text-[.9rem] uppercase font-semibold text-[#7e8299] tracking-wide"
        ...
        <li
          class="px-[25px] py-[9px] min-h-[44px] cursor-pointer relative flex flex-grow items-center text-[#3f4254] hover:bg-slate-100 hover:text-primary">
          <a class="flex items-center hover:text-current"><span :class="!isExpandSidebar && 'md:w-min'"
              class="flex w-[35px]"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
              ...
              </svg></span><span :class="!isExpandSidebar && 'md:hidden'">Keluar</span></a>
        </li>
      </ul>
    </div>
  </div>
